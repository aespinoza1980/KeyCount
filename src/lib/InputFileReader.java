...
public class InputFileReader {
    ...
    /**
     * Returns number of occurrences of character in a String sentences
     *
     * @param stringLine contains the string from which we wish to read the  character
     * @param searchFor contains the character to serch for in the string
     * @return int (number of occurrences of searchFor in the stringLine)
     */
    private static int countOccurrencesOf(String stringLine, char searchFor)
    {
        int count = 0;
        for (int i=0; i < stringLine.length(); i++)
        {
            if (stringLine.charAt(i) == searchFor)
            {
                count++;
            }
        }
    ...
    private static boolean isLineFormatCorrect(String line){
        ...
        if(line.contains(",")) {
            ...
            if (((parts.length == 2) && (isInteger(parts[1].replaceAll("\\s+", ""))) && (countOccurrencesOf(line, ',')) == 1))
                isLineCorrect = true;
        }
